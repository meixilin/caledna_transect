...
sink(paste0(outdir, "logs/pcoa_nocoast_plot_c4", Sys.Date(), ".log"))
...
myplot_ordination <- function(forplot, forplotdf, mycolours, groupv, primer){
    ...
    ggsave(paste0("beta_diver_pcoa_nocoast_", groupv, ".pdf"), plot = pp, path = plotdir, width = 10, height = 6) # could be clear if needed 
    pp1 <- pp + 
        theme(legend.position = "none")
    ggsave(paste0("beta_diver_pcoa_nocoast_", groupv, "_noleg.pdf"), plot = pp1, path = plotdir, width = 6, height = 6)
    return(pp)
}

# load data -------
load(file = "./derive_data/phy_rare/phyrare.RData")
jadiss <- loadRData(file = paste0(outdir, "nocoast/jadiss_nocoast.RData"))
...
nocoastdir = paste0(outdir, "nocoast/")
dir.create(nocoastdir, recursive = T)
save(pcoalist, file = paste0(nocoastdir, "pcoa_nocoast_result.RData"))
save(pplist, file = paste0(nocoastdir, "pcoa_nocoast_category_plot.RData"))
...
# for all 5 primers_commeco, there were negative eigenvalues in PITS but not the others. No corrections were performed for PITS primer. 
# 