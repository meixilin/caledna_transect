...
surf_dtprep <- function(ii) {
    ...
    # combine pch values 
    pchs = dplyr::left_join(x = pchs, y = mypch, by = groupv)
...
surf_plot_novectors <- function(to_return, primer, myvar, myconf, plotdir = NULL, legend = TRUE) {
    ...
    arrows(x0 = 0, y0 = 0, 
           ...
           col = mycol, lwd = 2)
    mp <- with(sampledf, ordisurf(ord.res, eval(as.name(myvar)), add = TRUE, labcex = 1.2))
    return(0)
}

# main ========
myvar = c("NDVI32", "elev")
pdf(file = paste0(plotdir, "beta_diver_envfit_surf_fig5.pdf"), height = 12, width = 12)
pp <- par(mfrow = c(2,2), cex = 1)
surf_plot_novectors(surf_dtprep(ii = 4), primer = primers_commeco[4], myconf = myconf, myvar = myvar[1], legend = F)
surf_plot_novectors(surf_dtprep(ii = 5), primer = primers_commeco[5], myconf = myconf, myvar = myvar[1], legend = F)
surf_plot_novectors(surf_dtprep(ii = 1), primer = primers_commeco[1], myconf = myconf, myvar = myvar[2], legend = T)
surf_plot_novectors(surf_dtprep(ii = 3), primer = primers_commeco[3], myconf = myconf, myvar = myvar[2], legend = F)
par(pp)
dev.off()
