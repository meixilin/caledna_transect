...
# Accompanying "predict_gf_Trns_CAgrid_noextrap_prcomp_RGB_20201025.pdf"
# Author: Meixi Lin (meixilin@ucla.edu)
# Date: Tue Apr 21 23:30:31 2020
# Author: Meixi Lin (meixilin@ucla.edu)
# Date: Sun Oct 25 18:25:15 2020
...
load(file = paste0(indir, gfname, "/", treatment, "/", "predict_gf_Trns_CAgrid_noextrap_prcomp_RGB.RData"))
Trns_CArgbid = data.table::setDF(Trns_CArgb[id,])
rm(Trns_CArgb)

# load the PCs
load(file = paste0(indir, gfname, "/", treatment, "/", "predict_gf_Trns_CAgrid_noextrap_prcomp.RData"))

# load the sites 
load(file = paste0(indir, gfname, "/", treatment, "/", "predict_gf_Trns_site_noextrap.RData"))
# main --------
# get the loadings 
...
pdf(file = paste0(plotdir, "fig9_pcs_loading_noextrap_CA_", today,".pdf"), height = 6, width = 6)
par(bg = NA)
...
text(min(xrng), 0, xlab, pos=3, col = "dimgrey")
text(0, min(yrng), ylab, pos=4, col = "dimgrey")
...
PCsites <- predict(PCs, Trns_site_noextrap)
