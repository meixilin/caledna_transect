# Title: generate summary for duplicated sites with full records --------
# Author: Meixi Lin
...
# read in data --------
rawdata <- loadRData(file = paste0(indir, "phy", rare, "_wdup.RData"))
...
for (ii in dup) {
    for (jj in primers) {
        summary.percent[ii, jj] <- count_overlap(subphy[[jj]], ii, prune = F)['percent']
    }
}

write.csv(summary.percent, file = paste0(outdir, txlevel, "_", rare, "_", cutoff, "_full.csv"))
